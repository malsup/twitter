...
;(function($) {
    $.fn.twitterSearch = function(options) {
        ...
        return this.each(function() {
            ...
            if (opts.tweetHeight) {
                opts.css['container'].height = 60;
                frameFontSize = ~~$frame.css('font-size').split('px')[0]
            }
            ...
            $.getJSONP({
                ...
                success: function(json) {
                    ...
                    $.each(json.results, function(i) {
                        ...
                        $tweet.appendTo($cont);
                        if (opts.tweetHeight && i === 0)
                            $cont.animate({ height: $tweet.height()+frameFontSize }, opts.animOutSpeed);
            ...
            function go() {
                ...
                $el.animate(opts.animOut, opts.animOutSpeed, function() {
                    if (opts.tweetHeight)
                        $cont.animate({ height: $el.next().height()+frameFontSize }, opts.animOutSpeed);
    ...
    $.fn.twitterSearch.defaults = {
        ...
        tweetHeight: false,            // if true, only one tweet will be display (by @Calvein)
        css: {
            // default styling
            ...
            frame: { border: '10px solid #C2CFF1', borderRadius: '10px', '-moz-border-radius': '10px', 'border-radius': '10px' },
